...
export class MandatePricingListComponent implements OnInit {
  ...
  brochureFileData: {
    [key: string]: { file: any; fileName: string; result: any };
  } = {};
  videoFileData: {
    [fileType: string]: {
      file: any;
  ...
  fileTypes = [['Common'], ['1 BHK', '2 BHK'], ['3 BHK', '4 BHK']];
  ...
  getBuilderList() {
    this.sharedService.builderlist1().subscribe((response) => {
      ...
      this.builderList = Array.isArray(response['Builders'])
        ? response['Builders']
        : [];
  ...
  onBrochureFileSelected(event) {
    if (
      ...
      event.target.files[0].size > 100 * 1024 * 1024
    ) {
      Swal.fire({
  ...
  onPriceSheetFileSelected(event, key, type) {
    ...
    if (
      event.target.files[0] &&
      event.target.files[0].size > 50 * 1024 * 1024
    ) {
      Swal.fire({
        title: 'File Size Exceeded',
  ...
  checkBHKBeforeFileUpload(event: MouseEvent, type) {
    if (
      (type === 'video' && !this.selectedVideoBHK) ||
      (type == 'floorPlan' && !this.selectedFloorPlanBHK)
    ) {
  ...
  onVideoFileSelected(event) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (
        ...
        event.target.files[i].size > 100 * 1024 * 1024
      ) {
        Swal.fire({
          title: 'File Size Exceeded',
          text: 'File Size limit is 100MB',
  ...
  viewVideo(video) {
    if (this.isEditProp && !video.result) {
      window.PreviewAnyFile.preview(
        'https://lead247.in/images/videos/' + video.fileName
      )
  ...
  onFloorPlanFileSelected(event) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (
        event.target.files[i] &&
        event.target.files[i].size > 10 * 1024 * 1024
      ) {
        Swal.fire({
          title: 'File Size Exceeded',
  ...
  onFloorePlanView(file: any) {
    ...
    if (this.isEditProp) {
      window.PreviewAnyFile.preview(
        'https://lead247.in/images/floorplans/' + file.fileName
      )
        .then((res) => console.log('success', res))
        .catch((err) => console.log('error', err));
    } else {
  ...
  deleteFloorPlan(index) {
    ...
    param = {
      ...
      floorplanName:
        this.floorPlanFileData[this.selectedFloorPlanBHK][index].fileName,
      detailid: this.editingProp.detailsId,
    };
    ...
    this._mandateService.deletefloorplan(param).subscribe((resp) => {
      Swal.fire({
        text:
          `PriceSheet Deleted Successfully for ` +
          this.floorPlanFileData[this.selectedFloorPlanBHK][index].bhkId +
          'BHK',
        heightAuto: false,
        icon: 'success',
        timer: 1000,
  ...
  updateFloorPlan_PropInfo() {
    if (this.builderPropertyid == '') {
      ...
      if (
        this.floorPlanFileData.Common &&
        this.floorPlanFileData.Common.length > 0
      ) {
        this.floorPlanFileData.Common?.forEach((floorPlanFile) => {
          formData.append('fp0bhk[]', floorPlanFile.file || '');
        });
        const sqftValues = this.floorPlanFileData['Common']
          .map((fp) => fp.sqft)
          .join(',');
      ...
      if (
        this.floorPlanFileData['1BHK'] &&
        this.floorPlanFileData['1BHK'].length > 0
      ) {
        this.floorPlanFileData['1BHK']?.forEach((floorPlanFile) => {
          formData.append('fp1bhk[]', floorPlanFile.file);
        });
        const sqftValues = this.floorPlanFileData['1BHK']
          .map((fp) => fp.sqft)
          .join(',');
      ...
      if (
        this.floorPlanFileData['2BHK'] &&
        this.floorPlanFileData['2BHK'].length > 0
      ) {
        this.floorPlanFileData['2BHK']?.forEach((floorPlanFile) => {
          formData.append('fp2bhk[]', floorPlanFile.file);
        });
        const sqftValues = this.floorPlanFileData['2BHK']
          .map((fp) => fp.sqft)
          .join(',');
      ...
      if (
        this.floorPlanFileData['3BHK'] &&
        this.floorPlanFileData['3BHK'].length > 0
      ) {
        this.floorPlanFileData['3BHK']?.forEach((floorPlanFile) => {
          formData.append('fp3bhk[]', floorPlanFile.file);
        });
        const sqftValues = this.floorPlanFileData['3BHK']
          .map((fp) => fp.sqft)
          .join(',');
      ...
      if (
        this.floorPlanFileData['4BHK'] &&
        this.floorPlanFileData['4BHK'].length > 0
      ) {
        this.floorPlanFileData['4BHK']?.forEach((floorPlanFile) => {
          formData.append('fp4bhk[]', floorPlanFile.file);
        });
        const sqftValues = this.floorPlanFileData['4BHK']
          .map((fp) => fp.sqft)
          .join(',');
  ...
  onEditProperty(prop) {
    ...
    prop.Pricesheets.forEach((pricesheet) => {
      ...
      if (key) {
        ...
        this.priceSheetFileData[key].fileName =
          pricesheet.propdetails_pricesheet || '';
      }
    });
  ...
  onPropertySearch() {
    this.propertyList1 = this.propertyList.filter((item) => {
      return item.PropName?.toLowerCase().includes(
        this.execPropName?.toLowerCase()
      );
    });
  }
}
