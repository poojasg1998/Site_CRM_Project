...
export class DashboardComponent {
  ...
  getScheduledplans_counts() {
    this.mandateService
      .getScheduledplans_counts(this.filteredParams1)
      .subscribe((resp) => {
        console.log(resp);
  ...
  setCalendarFromStoredDates() {
    if (
      this.filteredParams1.weekendfromdate &&
      this.filteredParams1.weekendtodate
    ) {
      const from = this.parseDate(this.filteredParams1.weekendfromdate);
      const to = this.parseDate(this.filteredParams1.weekendtodate);

      this.plan_vs_summary_dateRange = [from, to];
    }
  }
  onBackButton() {
  ...
  isWeekday = (dateString: string) => {
    ...
    return this.filteredParams1.plan == '1'
      ? utcDay !== 0 && utcDay !== 6
      : utcDay === 0 || utcDay === 6;
  };
  isWeekend = false;
  ...
  onPlan_vs_summary_dateRange() {
    ...
    this.filteredParams1.weekendtodate =
      this.plan_vs_summary_dateRange[1].toLocaleDateString('en-CA');
    console.log(this.filteredParams1);
    this.addQueryParams();
  }
  onScroll1(event: any) {
